...
//
// **미로 크기 재확대 + "방향 결정력" 지표 폐기 (2026-09-16)**: 실제 테스트
// 참가자 3명이 전부 같은 의견 — "너무 쉽고 미로 난이도가 쉽다". 동시에
// index.html에서 랜드마크 위치·목적지 힌트를 전부 지웠으므로(참가자가 더는
// "목표 쪽 방향"을 알 방법이 없음), 바로 위 단락의 "방향 결정력" 지표는 갈
// 곳을 잃은 지표가 됐다(그 정보를 아는 참가자가 없으니 갈림길이 방향으로
// 풀리든 말든 무의미). 그래서 이번엔 그 지표를 버리고, 갈림길 밀도·가지
// 길이 균질성만 기준으로 재탐색. 15x15(25칸)를 17x17(7x7칸->8x8칸)로 한
// 단계 키워 seed=25 채택 — 경로 33칸, 갈림길 12개(전체 19개), 최장 가지
// 6칸. 완벽 주행 시간이 약 27초->37초로 늘어난다(이전 19x19 버전과 거의
// 같은 체감 크기이지만 격자는 더 작음 — Prim's algorithm 특유의 변동성으로
// 같은 경로 길이도 격자 크기에 따라 다른 시드에서 나옴).
//
// **한 단계 더 확대 — 21x21 (2026-09-17)**: 17x17로도 "맵 3종이 회전/반전
// 관계라는 걸 눈치채는 것 같다"는 피드백 — 3개 맵이 진짜로 같은 미로(He et
// al. 2024 기법)라 어쩔 수 없는 구조적 특성이지만, 미로가 커질수록 참가자가
// 실제로 걷는 구간이 전체에서 차지하는 비중이 작아지고 기억해야 할 turn
// 수가 늘어나 "봤던 패턴이다"를 의식적으로 알아채기 어려워진다. 난이도
// 상승(1.6배, 시뮬레이션 확인)과 겹치는 효과라 함께 적용. 10x10칸(21x21
// 격자)에서 갈림길 밀도·가지 길이로 재탐색해 seed=86 채택 — 경로 41칸,
// 갈림길 12개(전체 29개), 최장 가지 6칸. 완벽 주행 시간 약 37초->46초.
// 맵 유사성·시작-도착이 항상 대각선이라는 구조는 여전히 남아있는 한계라
// docs/03에 기록함(회전/반전 기법을 버리면 "난이도 동일 증명"이 깨져서
// 더 큰 트레이드오프가 됨).

const MAZE_CELL_N = 10;   // 10x10칸짜리 미로 (칸과 칸 사이 벽을 포함하면 21x21 격자)
const MAZE_SEED = 86;     // 고정 시드 — 경로 41칸, 갈림길 12개(전체 29개), 최장 가지 6칸
const GRID_SIZE = MAZE_CELL_N * 2 + 1; // 21x21, 테두리는 항상 벽
...
function buildBaseGrid() {
  ...
  // 처음엔 이 구조("소리 없는 대각선 반대편 모서리가 목적지")를 안내 화면에서
  // 직접 알려줬는데, 실제 플레이 피드백으로 "너무 큰 힌트"라는 게 드러남 —
  // 랜드마크 위치 3곳(모서리 4개 중 3개)만 알아도 나머지 하나(목적지)가
  // 뺄셈으로 바로 나와서, 랜드마크 개수(0/2/3)와 무관하게 항상 목적지 방향을
  // 알 수 있는 구조적 비콘이 되어버렸음. 그래서 index.html에서 이 규칙과
  // 랜드마크 정확한 위치 둘 다 안내문에서 뺐다(참가자는 "고정된 소리 세 곳"
  // 정도만 알고 직접 방향을 가늠해야 함) — 좌표 배치 자체(삼각측량 품질,
  // 진행도 비콘 방지)는 그대로 유효하므로 바꾸지 않았다.
  //
  ...
  //
  // **거리 감쇠를 연속 dB → 하드 컷오프로 교체하면서도 8칸 오프셋은 유지 (2026-09-16)**:
  // 참가자 3명 전원이 "소리가 항상 들려서 의미 없이 거슬리기만 한다"는 의견 —
  // 이전 감쇠 모델(inverse)은 스펙상 아무리 멀어도 완전히 무음이 되지 않고 바닥값에
  // 수렴만 하기 때문. `config.js`에서 linear 모델(특정 거리 밖은 진짜 무음, 컷오프
  // 거리 = LANDMARK_MAX_DISTANCE = 60유닛 = 직선거리 약 15칸)로 바꿨다. 이 새
  // 모델 아래서 2번의 8칸 오프셋이 여전히 유효한지 재확인함 — 경로를 따라
  // "들림/안 들림"이 몇 번 바뀌는지, 그 전환이 진행도와 얼마나 상관되는지
  // 계산해보니, 오프셋이 클수록(=랜드마크가 멀수록) 들리는 구간 자체가 짧아져서
  // 진행도 정보가 새더라도 노출 범위가 작게 묶임(17x17에서는 경로의 약 12%,
  // 21x21로 키운 뒤에도 재확인해서 17%로 비슷한 수준 — 아래 2026-09-17 참고).
  // 오프셋을 줄이면 들리는 구간이 늘면서 상관도 같이 커져 더 나빠짐 — 8칸을
  // 그대로 유지.
  //
  // **21x21로 확대 후 컷오프 거리 재확인, 60유닛 유지 (2026-09-17) — 이후 오류로
  // 밝혀짐**: 40~105유닛 범위로 재계산해서 60유닛이 여전히 괜찮다고 판단했었는데,
  // 이때 **최단경로 위 거리만** 계산한 게 실수였다. 이 시드의 최단경로가 우연히
  // 양옆 랜드마크에 가깝게 지나가서 "60유닛도 괜찮아 보이는" 착시가 있었음.
  //
  // **미로 전체 기준으로 재검증, 90유닛으로 수정 (2026-09-18)**: 실제 플레이
  // 피드백 "조금만 걸어도 소리가 안 들림". 참가자는 최단경로만 걷지 않고 미로
  // 전체를 헤매므로, 최단경로가 아니라 **미로 전체 열린 칸** 기준으로 다시 계산해야
  // 했다. 그랬더니 양옆 랜드마크(0/1)는 **시작 지점에서부터 이미 72유닛** 떨어져
  // 있었다 — 삼각측량을 위해 시작과 먼 두 모서리에 배치했기 때문에 생기는 당연한
  // 결과인데, 60유닛 컷오프면 시행이 시작되자마자 이미 거의 안 들리는 상태였던
  // 것(미로 전체 기준 51~54%만 들림). LANDMARK_MAX_DISTANCE를 90유닛으로 올리자
  // 양옆 랜드마크가 미로 전체의 95%(최단경로는 100%)에서 들리면서 진행도 상관은
  // 0으로 유지됨을 재확인. 바람 소리(2번)는 여전히 노출이 좁은데(미로 전체
  // 28%), 이건 의도한 그대로다 — 계기판 방지를 위해 일부러 노출을 좁힌 랜드마크라
  // 양옆 랜드마크와 같은 수준으로 들릴 필요가 없다.
  const landmarks = [
