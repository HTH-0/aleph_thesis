...
function buildBaseGrid() {
  ...
  // 그대로 유지.
  //
  // **21x21로 확대 후 컷오프 거리 재확인 (2026-09-17)**: 미로가 커지면 대각선도
  // 늘어나므로(17x17 약 90유닛 -> 21x21 약 113유닛) LANDMARK_MAX_DISTANCE(60)도
  // 같이 늘려야 하는지 40~105유닛 범위로 다시 계산해봤다. 60유닛이 새 크기에서도
  // 비슷한 균형을 냄 — 양옆 랜드마크(0/1) 노출 73%/29%(상관 0.29/0.03), 바람
  // 소리(2) 노출 17%(상관 -0.64지만 노출 자체가 작아 실제로 새는 정보량은 작음).
  // 그래서 컷오프 거리는 미로 크기와 무관하게 60유닛(절대 거리)으로 그대로 둠 —
  // 참가자가 느끼는 "가까우면 들리고 멀면 안 들린다"는 감각을 미로 크기와
  // 무관하게 일정하게 유지하는 게, 미로 크기에 비례해서 계속 바꾸는 것보다
  // 더 직관적이라고 판단함.
  const landmarks = [
