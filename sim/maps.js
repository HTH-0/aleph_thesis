...
const MAZE_CELL_N = 8;    // 8x8칸짜리 미로 (칸과 칸 사이 벽을 포함하면 17x17 격자)
const MAZE_SEED = 25;     // 고정 시드 — 경로 33칸, 갈림길 12개(전체 19개), 최장 가지 6칸
...
function buildBaseGrid() {
  ...
  // 수렴만 하기 때문. `config.js`에서 linear 모델(특정 거리 밖은 진짜 무음)로
  // 바꿨다. 이 새 모델 아래서 2번의 8칸 오프셋이 여전히 유효한지 재확인함 —
  // 경로를 따라 "들림/안 들림"이 몇 번 바뀌는지, 그 전환이 진행도와 얼마나
  // 상관되는지 계산해보니, 오프셋이 클수록(=랜드마크가 멀수록) 들리는 구간
  // 자체가 짧아져서(현재 17x17 미로 기준 전체 경로의 약 12%만 들림) 진행도
  // 정보가 새더라도 노출 범위가 작게 묶임. 오프셋을 줄이면 들리는 구간이
  // 늘면서 상관도 같이 커져 더 나빠짐 — 8칸을 그대로 유지.
  const landmarks = [
