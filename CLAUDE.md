...
5. **`trial.js`** (`runTrial`) — owns one Three.js scene/run of the maze: builds wall/floor/ceiling meshes from the grid (uniform materials + fog, deliberately no textures — visual landmarks would confound the audio manipulation), handles WASD+pointer-lock movement and wall collision, and logs elapsed time, distance traveled, path efficiency, dead-end entries, and reorientation time (time spent stationary while rotating the camera — a proxy for "listening to figure out direction"). **The WebGLRenderer is created once and reused across trials** (`getSharedRenderer`) — recreating it per trial previously caused the canvas to hang on the second trial. Handles `pointerlockchange` to pause the trial (and show the overlay again) if the browser force-exits pointer lock, e.g. on alt-tab — without this, mouse look silently stopped working while WASD kept moving the player. Visiting with `?test=1` in the URL enables dev/QA-only features (inert without the query param): a `P` hotkey that instantly finishes the current trial (for exercising the full flow — including the server save — without walking each maze), and a top-left HUD showing each active landmark's live distance and computed volume (re-implements the PannerNode `linear` distance-model formula from `CONFIG.LANDMARK_*`, since the browser doesn't expose the value it's actually using) — for confirming the falloff/cutoff behaves as configured without having to judge it by ear.
...
8. **`api/results.js`** — Vercel serverless function, gated by a `?key=` query param matched against the `ADMIN_KEY` env var. `GET` returns all rows as JSON; `DELETE` (with `&id=<row id>`) removes one row — used by `admin.html`'s per-row delete button to clear out test/QA submissions. Not linked from any participant-facing screen.
